...
def compute_features_for_window(
    ...
    for i, col in enumerate(value_cols):
        ...
        complexity = float(np.sqrt(var_ddx / var_dx) / (mobility + 1e-12))
        features[f"{col}_hjorth_activity"] = var_x
        features[f"{col}_hjorth_mobility"] = mobility
        features[f"{col}_hjorth_complexity"] = complexity

        # --- Engagement & attention indices (per channel) ---
        beta_p = bp["beta"] if bp["beta"] > 0 else 1e-9
        theta_p = bp["theta"] if bp["theta"] > 0 else 1e-9
        # Pope's engagement index: beta / (alpha + theta)
        features[f"{col}_engagement"] = float(bp["beta"] / (bp["alpha"] + bp["theta"] + 1e-12))
        # Theta/Beta ratio: classical inverse-attention marker
        features[f"{col}_theta_beta_ratio"] = float(bp["theta"] / beta_p)
        # Cognitive load index: theta / alpha (frontal theta proxy)
        features[f"{col}_load_index"] = float(theta_p / alpha_p)

    # --- Channel-pair asymmetry features (proxy for hemispheric asymmetry) ---
    # Without explicit montage, pair channel i with channel (N-1-i) as a heuristic.
    n_ch = len(value_cols)
    for i in range(n_ch // 2):
        j = n_ch - 1 - i
        ci, cj = value_cols[i], value_cols[j]
        a_i = features.get(f"{ci}_bp_alpha", 0.0)
        a_j = features.get(f"{cj}_bp_alpha", 0.0)
        # log asymmetry index (classical FAA formula)
        features[f"asym_alpha_{ci}_{cj}"] = float(
            np.log(a_j + 1e-9) - np.log(a_i + 1e-9)
        )
        b_i = features.get(f"{ci}_bp_beta", 0.0)
        b_j = features.get(f"{cj}_bp_beta", 0.0)
        features[f"asym_beta_{ci}_{cj}"] = float(
            np.log(b_j + 1e-9) - np.log(b_i + 1e-9)
        )
