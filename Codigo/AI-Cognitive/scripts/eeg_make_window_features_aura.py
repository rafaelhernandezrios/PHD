...
# AURA channel mapping (documented in Cognitive-load/README.md)
AURA_CHANNELS = {
    "v1": "Fp1", "v2": "Fp2", "v3": "F3", "v4": "Fz",
    "v5": "F4",  "v6": "P3",  "v7": "Pz", "v8": "P4",
}
EEG_COLS_RAW = list(AURA_CHANNELS.keys())            # v1..v8
EEG_COLS = [AURA_CHANNELS[c] for c in EEG_COLS_RAW]  # Fp1..P4

# Frontal/parietal subsets for the load index and asymmetries
FRONTAL = ["F3", "Fz", "F4"]
PARIETAL = ["P3", "Pz", "P4"]
LR_PAIRS = [("Fp1", "Fp2"), ("F3", "F4"), ("P3", "P4")]
...
def compute_window_features(segment: np.ndarray) -> Dict[str, float]:
    ...
    # Classical cognitive load index: Theta(Fz) / Alpha(Pz)
    theta_fz = bp_by_channel["Fz"]["theta"]
    alpha_pz = bp_by_channel["Pz"]["alpha"] + 1e-9
    feats["frontal_theta_parietal_alpha"] = float(theta_fz / alpha_pz)

    # Mean band powers across frontal vs parietal sites
    for band in ("theta", "alpha", "beta"):
        f_mean = np.mean([bp_by_channel[c][band] for c in FRONTAL])
        p_mean = np.mean([bp_by_channel[c][band] for c in PARIETAL])
        feats[f"frontal_mean_{band}"] = float(f_mean)
        feats[f"parietal_mean_{band}"] = float(p_mean)
        feats[f"front_minus_parietal_{band}"] = float(f_mean - p_mean)
...
def main() -> None:
    ...
    rename = {raw: name for raw, name in AURA_CHANNELS.items()}
