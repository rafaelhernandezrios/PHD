...
def main():
    ...
    print(f"Directorio electron (extra): {ELECTRON_DATA_DIR}")
    ...
    # Buscar archivos CSV en data/Data-Experimento-Rafa y electron/data_*
    csv_files = []
    subject_seen = set()
    candidate_dirs = [Path(DATA_DIR), Path(ELECTRON_DATA_DIR)]
    for candidate_dir in candidate_dirs:
        if not candidate_dir.exists():
            print(f"ADVERTENCIA: No se encontro el directorio {candidate_dir}")
            continue
        for data_dir in sorted(candidate_dir.glob('data_*')):
            subject_name = data_dir.name.replace('data_', '').strip().lower()
            if subject_name in subject_seen:
                continue
            subject_csv_files = sorted(data_dir.glob('eeg_data_*.csv'))
            if not subject_csv_files:
                continue
            # Mantener el CSV mas reciente por sujeto (si hubiera multiples sesiones)
            csv_files.append(subject_csv_files[-1])
