"""
Deep-dive pipeline walk-through for subject "michelle".
...
Output: output/deep_dive/michelle/
"""
...
DATA_CSV = BASE_DIR / "data" / "Data-Experimento-Rafa" / "data_michelle" / "eeg_data_20260414_182434.csv"
OUT_DIR = BASE_DIR / "output" / "deep_dive" / "michelle"
...
def plot_global_timeline():
    ...
    fig.suptitle("michelle — raw EEG across all 8 channels, phases shaded", fontsize=13)
    plt.tight_layout()
...
def plot_step_by_step_per_phase():
    ...
    for ph in phases:
        ...
        fig.suptitle(f"michelle — {ph} (sr≈{sr:.1f} Hz, dur={r['dur']:.1f} s)",
...
def plot_psd_overlay():
    ...
    for j, (ax, ch_name, key_clean) in enumerate(
        ...
        ax.legend(loc="upper right", fontsize=8)
        ax.grid(alpha=0.3, which="both")
    fig.suptitle("michelle — PSD of cleaned signals (Fz & Pz) across 4 phases", fontsize=13)
    plt.tight_layout()
...
def plot_windowed_ratios():
    ...
    fig.suptitle("michelle — windowed Theta/Alpha ratio per phase (two SR modes)",
                 fontsize=13, fontweight="bold")
    plt.tight_layout()
...
def plot_summary_bars():
    ...
    ax.set_title("michelle — mean ratio per phase (actual SR vs step4 assumption)")
    ax.legend()
...
def plot_abs_powers():
    ...
    fig.suptitle("michelle — Theta(Fz) and Alpha(Pz) power per phase", fontsize=13, fontweight="bold")
    plt.tight_layout()
...
def save_window_csv():
    ...
    pd.DataFrame(rows).to_csv(OUT_DIR / "michelle_windows.csv", index=False)
    ...
    pd.DataFrame(summary_rows).to_csv(OUT_DIR / "michelle_summary.csv", index=False)
    log("  Saved michelle_windows.csv + michelle_summary.csv")
